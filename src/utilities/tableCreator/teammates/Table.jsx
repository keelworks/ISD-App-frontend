...
const Table = ({ tableData, setMembers }) => {
  ...
  useEffect(() => {
    if (isFetching === false && isSuccess === true) {
      setMembers(transformMembersData(fetchedMembers));
    }
  }, [fetchedMembers]);

  const submitFormFunc = async (members, member) => {
    try {
  ...
  const renderHeaderSorterIcons = (header) => {
    ...
    if (isSortable) {
      return (
        <>
          {{
            asc: <img src={ArrowUp} alt="Arrow" className="arrow-icon" />,
            desc: <img src={ArrowDown} alt="Arrow" className="arrow-icon" />,
      ...
      return (
        <div
          className={`status-icon ${
            cell.row.original.status ? "active-status" : "inactive-status"
          }`}
        >
          <span>&bull;</span>
  ...
  const handleKeyDown = (e, table, row, setIsEditing) => {
    setIsEditing(true);
    editMember(e, table, row, inputErrors, data, submitFormFunc);
  };
